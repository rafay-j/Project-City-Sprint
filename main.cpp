...
void Path(float d, int e, int st, int inter, stack<int> &pathStack)
{
    int t = e, s;
    static float dist = 0;
    dist += d;
...
int printSolution(float dist[], int n, int src, int temp, int inter, stack<int> &pathStack)
{
    Details(temp);
    Path(dist[temp], temp, src, inter, pathStack);
}

void dijkstra(float graph[V][V], int src, int targ, int inter, stack<int> &pathStack)
{
    float dist[V];
    bool sptSet[V];
    for (int i = 0; i < V; i++)
    ...
    printSolution(dist, V, src, targ, inter, pathStack);
}
...
{		
        ...
        if(interflag1){
          dijkstra(graph,startcode,intercode,0,pathStack);
          dijkstra(graph,intercode,endcode,1,pathStack);
          }
        else
          dijkstra(graph, startcode,endcode,1,pathStack);
}
